...
struct VoiceAnythingContractTests {
    ...
    @Test func bundledHardwareAndCustomControlIDsDecodeWithoutEnumChanges() throws {
        ...
        #expect(profile.transport?.vendorId == 0x2717)
        #expect(profile.transport?.productId == 0x32b8)
        #expect(profile.matchesModel(" rc003ms "))
        #expect(!profile.matchesModel("OTHER-RC003"))
