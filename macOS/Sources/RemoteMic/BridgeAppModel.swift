...
final class BridgeAppModel: ObservableObject, XiaomiBluetoothBridgeDelegate {
    let voiceAnythingJournal: VoiceAnythingJournal?
    private var voiceAnythingPrivacySubscription: AnyCancellable?
    ...
    init(
        ...
        voiceAnythingJournal: VoiceAnythingJournal? = nil
    ) {
        self.voiceAnythingJournal = voiceAnythingJournal
        ...
        if let voiceAnythingJournal {
            settings.localTranscriptHistoryEnabled = false
            settings.onVoiceAnythingButton = { date in voiceAnythingJournal.recordButton(at: date) }
            voiceAnythingPrivacySubscription = voiceAnythingJournal.$document
                .map(\.recordReflections).removeDuplicates().receive(on: RunLoop.main)
                .sink { [weak settings] enabled in settings?.localTranscriptHistoryEnabled = enabled }
        }
    ...
    private func archiveCapturedTranscript(_ capture: CapturedTranscript) {
        ...
        if let voiceAnythingJournal {
            voiceAnythingJournal.append(sessionID: capture.sessionID, start: capture.startedAt, end: capture.endedAt,
                application: capture.applicationName, model: "xiaomi-atvv-v1", text: transcript)
            return
        }
    ...
    private func endVoiceSessionIfNeeded(flushAudio: Bool = true) {
        ...
        if let voiceSessionStartedAt {
            voiceAnythingJournal?.recordVoice(start: voiceSessionStartedAt, end: endedAt,
                audioSeconds: Double(voiceAudioDeliveryDiagnostic.receivedSamples) / 16_000)
