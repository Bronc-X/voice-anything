...
final class HardwareAnnouncementStore: ObservableObject {
    ...
    func refresh() {
        // Fork builds do not contact the upstream product's announcement service.
        guard Bundle.main.object(forInfoDictionaryKey: "VAHardwareAnnouncementsEnabled") as? Bool == true else { return }
        task?.cancel()
        requestGeneration &+= 1
