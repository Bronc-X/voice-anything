...
final class AppSettings: ObservableObject {
    var onVoiceAnythingButton: ((Date) -> Void)?
    ...
    func configuredAction(
        ...
        guard supportsManifestGesture(button, trigger: trigger, profileID: selectedRemoteProfileID) else { return .disabled }
    ...
    func configuredAction(
        ...
        guard supportsManifestGesture(button, trigger: trigger, profileID: profileID) else { return .disabled }
        guard let profileID, profileID != selectedRemoteProfileID,
              let profile = remoteDeviceProfiles.first(where: { $0.id == profileID })
    ...
    private func supportsManifestGesture(_ button: RemoteButton, trigger: ButtonTrigger, profileID: UUID?) -> Bool {
        guard let profile = VoiceAnythingDevices.shared.profile(for: profileID) else { return true }
        let name = trigger == .singleClick ? "single" : trigger == .doubleClick ? "double" : "long"
        return profile.profile.controls.contains { $0.button == button && $0.gestures.contains(name) }
    }
